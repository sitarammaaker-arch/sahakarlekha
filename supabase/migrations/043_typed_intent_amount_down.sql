-- 043 down · revert typed intent/posting-request money: restore the 042 function (jforms
-- typed, intents/requests plain), then drop the columns. JSONB untouched — no data loss.
...
alter table procurement_financial_intents drop column if exists "amountAmountMinor";
alter table procurement_financial_intents drop column if exists "amountCurrency";
alter table procurement_posting_requests  drop column if exists "amountAmountMinor";
alter table procurement_posting_requests  drop column if exists "amountCurrency";
