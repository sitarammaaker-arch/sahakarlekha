-- ============================================================
-- SahakarLekha — Member Portal: extend member_portal_snapshot() with the member's accrued LOAN
-- INTEREST (069 loan_interest_accruals). Run in Supabase SQL Editor AFTER 069.
--
-- Same security model as 064/065/067, unchanged: zero parameters, identity from auth.uid() only,
-- SECURITY DEFINER + search_path '', every read scoped to the linked society + member, soft-deleted
-- rows excluded (RULE 5). The portal computes "interest due" with the SAME loanInterestDue the Loan
-- Register, KCC page and staff Member-360 use (RULE 2). Additive: every 067 key is returned exactly
-- as before; only 'loanAccruals' and 'loanInterestVouchers' are added. Society-level accrual
-- journals leave as an id only.
-- ============================================================
...
begin
  ...
  return jsonb_build_object(
    ...
    -- ── Loan interest (069/070) ── inputs of loanInterestDue (Loan Register = Member-360 = portal).
    -- THIS member's loan + KCC ids; accrual rows of those loans only (live, not deleted).
    'loanAccruals', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', a.id, 'loanId', a."loanId", 'amount', a.amount, 'overdue', a.overdue, 'voucherId', a."voucherId"
      ) order by a."periodTo")
      from public.loan_interest_accruals a
      where a.society_id = v_link.society_id and coalesce(a."isDeleted", false) = false
        and a."loanId" in (
          select l.id from public.loans l
          where l.society_id = v_link.society_id and l."memberId" = v_member.id and coalesce(l."isDeleted", false) = false
          union
          select k.id from public.kcc_loans k
          where k.society_id = v_link.society_id and k."memberId" = v_member.id)
    ), '[]'::jsonb),
    -- The vouchers the due rule reads: the accrual journals of those rows (ID ONLY — they are
    -- society-level, so no amount/lines/narration leave), and the live repayment / reserve-release
    -- vouchers of THIS member's loans (lines reduced to account/type/amount).
    'loanInterestVouchers', coalesce((
      select jsonb_agg(x.j) from (
        select jsonb_build_object('id', v.id, 'amount', 0, 'lines', '[]'::jsonb) as j
        from public.vouchers v
        where v.society_id = v_link.society_id and coalesce(v."isDeleted", false) = false
          and v.id in (
            select a."voucherId" from public.loan_interest_accruals a
            where a.society_id = v_link.society_id and coalesce(a."isDeleted", false) = false and a."voucherId" is not null
              and a."loanId" in (
                select l.id from public.loans l
                where l.society_id = v_link.society_id and l."memberId" = v_member.id and coalesce(l."isDeleted", false) = false
                union
        ...
        select jsonb_build_object(
          'id', v.id, 'refType', v."refType", 'refId', v."refId",
          'debitAccountId', v."debitAccountId", 'creditAccountId', v."creditAccountId", 'amount', v.amount,
          'lines', coalesce((
            select jsonb_agg(jsonb_build_object('accountId', l.value ->> 'accountId', 'type', l.value ->> 'type', 'amount', l.value -> 'amount'))
            from jsonb_array_elements(case when jsonb_typeof(v.lines) = 'array' then v.lines else '[]'::jsonb end) l
          ), '[]'::jsonb)) as j
        from public.vouchers v
        where v.society_id = v_link.society_id and coalesce(v."isDeleted", false) = false
          and v."refType" in ('loan.repayment', 'loan.interest.release')
          and v."refId" in (
            select l.id from public.loans l
            where l.society_id = v_link.society_id and l."memberId" = v_member.id and coalesce(l."isDeleted", false) = false
            union
...
-- 2. The 067 keys AND the new interest keys are in the function (expect: true):
--    select prosrc like '%dividendVouchers%' and prosrc like '%loanAccruals%' and prosrc like '%loanInterestVouchers%'
--    from pg_proc where proname = 'member_portal_snapshot';
-- 3. Every column the interest part reads exists (expect: 0 rows):
--    select c.t || '.' || c.col as missing from (values
--      ('loan_interest_accruals','loanId'),('loan_interest_accruals','amount'),('loan_interest_accruals','overdue'),
--      ('loan_interest_accruals','voucherId'),('loan_interest_accruals','periodTo'),('loan_interest_accruals','isDeleted'),
--      ('kcc_loans','memberId'),('loans','memberId'),('loans','isDeleted'),('vouchers','refType'),('vouchers','refId')
--    ) as c(t, col)
