-- 042 · T-05 — typed money columns for procurement_jforms (J-Form gross / deductions / net).
--
-- The J-Form is the statutory procurement settlement document, and its three money fields
-- lived ONLY in JSONB (CL-3 wants integer minor units in typed, constrained columns).
-- Same pattern as migration 017 (procurement_settlements): additive typed pair per field
-- (`<field>AmountMinor` bigint paise + `<field>Currency`), idempotent backfill, JSONB retained.
--
-- ONE DIFFERENCE from 017: J-Form rows are written ONLY by the atomic RPC
-- procurement_commit_transaction (client-side dual-write is impossible), so this migration
-- also replaces that function with a version whose jforms INSERT derives the typed columns
-- server-side from the same JSONB payload. The payload contract, numbering, settlements,
-- events and return shape are UNCHANGED — the insert simply fills six more columns.
-- Reversible: 042_down restores the previous function body and drops the columns.

-- 1. Typed columns (additive, nullable — a NULL means "not yet backfilled", dual-read falls back).
alter table procurement_jforms add column if not exists "grossAmountMinor"      bigint;
alter table procurement_jforms add column if not exists "grossCurrency"         text;
alter table procurement_jforms add column if not exists "deductionsAmountMinor" bigint;
alter table procurement_jforms add column if not exists "deductionsCurrency"    text;
alter table procurement_jforms add column if not exists "netAmountMinor"        bigint;
alter table procurement_jforms add column if not exists "netCurrency"           text;

-- 2. Idempotent backfill from the JSONB (rupees × 100 → paise; only rows not yet backfilled).
update procurement_jforms set
  "grossAmountMinor"      = coalesce("grossAmountMinor",      (round(((gross->>'amount')::numeric)        * 100))::bigint),
  "grossCurrency"         = coalesce("grossCurrency",         gross->>'currency',        'INR'),
  "deductionsAmountMinor" = coalesce("deductionsAmountMinor", (round(((deductions->>'amount')::numeric)   * 100))::bigint),
  "deductionsCurrency"    = coalesce("deductionsCurrency",    deductions->>'currency',   'INR'),
  "netAmountMinor"        = coalesce("netAmountMinor",        (round(((net->>'amount')::numeric)          * 100))::bigint),
  "netCurrency"           = coalesce("netCurrency",           net->>'currency',          'INR')
where "grossAmountMinor" is null or "netAmountMinor" is null or "deductionsAmountMinor" is null;

-- 3. Dual-write inside the atomic commit: same function, jforms INSERT extended with the six
--    typed columns derived from the SAME payload values (NULL-safe: a missing money object
--    leaves the typed columns NULL and dual-read falls back to the JSONB).
--    `create or replace` (no drop) so the existing GRANT to authenticated is preserved.
