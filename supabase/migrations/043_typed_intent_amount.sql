-- 043 · T-05 — typed money columns for procurement_financial_intents + procurement_posting_requests.
--
-- Both tables carry a single Money object in an `amount` JSONB column, and both are written
-- ONLY by the atomic procurement_commit_transaction RPC — the same shape migration 042 handled
-- for J-Forms. Additive typed pair per the `<field>AmountMinor` convention (field 'amount' →
-- "amountAmountMinor" bigint paise + "amountCurrency"), idempotent backfill, JSONB retained,
-- and the RPC's two INSERTs extended to derive the typed columns server-side. Payload contract,
-- numbering, settlements, events and return shape are UNCHANGED. Reversible via 043_down
-- (restores the 042 function body and drops the columns).

-- 1. Typed columns (additive, nullable — NULL means "not yet backfilled", dual-read falls back).
alter table procurement_financial_intents add column if not exists "amountAmountMinor" bigint;
alter table procurement_financial_intents add column if not exists "amountCurrency"    text;
alter table procurement_posting_requests  add column if not exists "amountAmountMinor" bigint;
alter table procurement_posting_requests  add column if not exists "amountCurrency"    text;

-- 2. Idempotent backfill from the JSONB (rupees × 100 → paise; only rows not yet backfilled).
update procurement_financial_intents set
  "amountAmountMinor" = coalesce("amountAmountMinor", (round(((amount->>'amount')::numeric) * 100))::bigint),
  "amountCurrency"    = coalesce("amountCurrency",    amount->>'currency', 'INR')
where "amountAmountMinor" is null;

update procurement_posting_requests set
  "amountAmountMinor" = coalesce("amountAmountMinor", (round(((amount->>'amount')::numeric) * 100))::bigint),
  "amountCurrency"    = coalesce("amountCurrency",    amount->>'currency', 'INR')
where "amountAmountMinor" is null;

-- 3. Dual-write inside the atomic commit: same function as 042, with the financialIntents and
--    postingRequests INSERTs extended (NULL-safe: absent money object → NULL typed columns →
--    dual-read falls back to the JSONB). `create or replace` preserves the existing GRANT.
