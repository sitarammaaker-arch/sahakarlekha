-- ============================================================
-- SahakarLekha — Member Portal 2b-2: extend member_portal_snapshot() with the member's GENERAL
-- DIVIDEND (Profit Distribution runs, 066). Run in Supabase SQL Editor AFTER 066.
--
-- Same security model as 064/065, unchanged: zero parameters, identity from auth.uid() only,
-- SECURITY DEFINER + search_path '', every read scoped to the linked society + member, soft-deleted
-- rows excluded (RULE 5). The portal computes with the SAME memberDividendHistory the staff
-- Member-360 and Profit Distribution use (RULE 2). Additive: every 065 key is returned exactly as
-- before; only 'dividendRuns' and 'dividendVouchers' are added.
--
-- Founder decisions: approved runs only, the member's own line only; legacy UNPAID years without a
-- frozen split stay hidden (decided client-side by memberDividendHistory).
-- ============================================================
...
-- 2. The 065 keys AND the new dividend keys are in the function (expect: true):
--    select prosrc like '%creditSales%' and prosrc like '%dividendRuns%' and prosrc like '%dividendVouchers%'
--    from pg_proc where proname = 'member_portal_snapshot';
-- 3. Every column the dividend part reads exists (expect: 0 rows):
--    select c.t || '.' || c.col as missing from (values
--      ('member_distribution_runs','fyLabel'),('member_distribution_runs','kind'),('member_distribution_runs','basis'),
--      ('member_distribution_runs','ratePct'),('member_distribution_runs','total'),('member_distribution_runs','status'),
--      ('member_distribution_runs','source'),('member_distribution_runs','lines'),('member_distribution_runs','isDeleted'),
--      ('vouchers','lines'),('vouchers','memberId'),('vouchers','narration'),('vouchers','createdAt'),('vouchers','isDeleted')
--    ) as c(t, col)
