// Ledger parity check (T-07 / ADR-0001) — READ-ONLY diagnostic. Reports, per society, whether
// replaying the `ledger_events` journal reproduces the balances of the current vouchers. This is the
// pre-flight for the T-09 cut: a tenant is only safe to make ledger-authoritative once it shows ✓.
// Never writes anything. PURE decision in src/lib/ledger/parity.ts (ledgerParity).
//
// Env: SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY (preferred; reads every tenant) or SUPABASE_ANON_KEY.
//   node scripts/check-ledger-parity.mjs
...
console.log(`\nLedger parity check (READ-ONLY) — key: ${USING_SERVICE ? 'service-role' : 'anon'}`);
...
for (const sid of [...societies].sort()) {
  ...
  if (p.matches) { okCount++; console.log(`  ✓ ${sid}  — ${p.accountsChecked} accounts agree`); }
    ...
    mismatchCount++;
    console.log(`  ✗ ${sid}  — ${p.diffs.length}/${p.accountsChecked} accounts differ:`);
    for (const d of p.diffs.slice(0, 8)) console.log(`       ${d.accountId}: vouchers ₹${toRupees(d.expected)} vs journal ₹${toRupees(d.actual)}`);
    if (p.diffs.length > 8) console.log(`       … and ${p.diffs.length - 8} more`);
  }
}

console.log(`\n${okCount} societ${okCount === 1 ? 'y' : 'ies'} ✓ ledger-ready, ${mismatchCount} with diffs.`);
console.log(mismatchCount === 0
  ? 'All societies match — the journal reproduces the vouchers. Safe to proceed to the T-09 cut per tenant.'
  : 'Some societies differ — run the genesis backfill (or investigate) before cutting those tenants.');
process.exit(0);
