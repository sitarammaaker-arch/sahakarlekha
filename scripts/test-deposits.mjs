...
function depositPosting(txnType, acc) {
  if (txnType === 'withdraw') return { debitAccountId: acc.liability, creditAccountId: acc.cashBank, sign: -1 };
  return { debitAccountId: acc.cashBank, creditAccountId: acc.liability, sign: 1 };
}
const applyDepositTxn = (balance, txnType, amount) => Math.round((balance + (txnType === 'withdraw' ? -1 : 1) * amount) * 100) / 100;
