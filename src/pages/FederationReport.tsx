...
const FederationReport: React.FC = () => {
  ...
  const fedDataRows = () => [
    ...
    ['3. Share Capital', 'Paid-up Capital (Rs.)', fmt(shareData.paidUp)],
    // Section 4
    ['4. Deposits', 'Total Deposits (Rs.)', fmt(depositTotal)],
    // Section 5
    ['5. Loans', 'Loans Sanctioned (Rs.)', fmt(loanData.sanctioned)],
    ['5. Loans', 'Loans Disbursed (Rs.)', fmt(loanData.disbursed)],
    ['5. Loans', 'Loans Recovered (Rs.)', fmt(loanData.recovered)],
    ['5. Loans', 'Loans Outstanding (Rs.)', fmt(loanData.outstanding)],
    ['5. Loans', 'Overdue Amount (Rs.)', fmt(loanData.overdue)],
    ['5. Loans', 'NPA Amount (Rs.)', fmt(loanData.npaAmt)],
    ['5. Loans', 'NPA %', pct(loanData.npaPct)],
    // Section 6
    ['6. Working Capital', 'Own Funds (Rs.)', fmt(wcData.ownFunds)],
    ['6. Working Capital', 'Borrowings (Rs.)', fmt(wcData.borrowings)],
    ['6. Working Capital', 'Total Working Capital (Rs.)', fmt(wcData.total)],
    // Section 7
    ['7. P&L Appropriation', 'Net Profit / (Loss) (Rs.)', fmt(plData.netProfit)],
    ['7. P&L Appropriation', 'Statutory Reserve Fund (Rs.)', fmt(plData.statutoryReserve)],
    ['7. P&L Appropriation', 'Education Fund (Rs.)', fmt(plData.educationFund)],
    ['7. P&L Appropriation', 'Dividend (Rs.)', fmt(plData.dividendFund)],
    ['7. P&L Appropriation', 'Other Fund Appropriations (Rs.)', fmt(plData.otherAppropriations)],
    ['7. P&L Appropriation', 'Balance carried forward (Rs.)', fmt(plData.balance)],
    // Section 8
