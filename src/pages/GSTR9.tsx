...
const GSTR9: React.FC = () => {
  ...
  const issues = useMemo(() => {
    ...
    const recs: GstCheckRecord[] = [
      ...(sales as unknown as Record<string, unknown>[]).filter(s => inFY(s.date as string | undefined)).map(s => ({ ref: `Sale ${(s.invoiceNo || s.saleNo || s.id) as string}`, netAmount: s.netAmount as number, cgstAmount: s.cgstAmount as number, sgstAmount: s.sgstAmount as number, igstAmount: s.igstAmount as number, cgstPct: s.cgstPct as number, sgstPct: s.sgstPct as number, igstPct: s.igstPct as number, isDeleted: s.isDeleted as boolean })),
      ...(purchases as unknown as Record<string, unknown>[]).filter(p => inFY(p.date as string | undefined)).map(p => ({ ref: `Purchase ${(p.billNo || p.purchaseNo || p.id) as string}`, netAmount: p.netAmount as number, cgstAmount: p.cgstAmount as number, sgstAmount: p.sgstAmount as number, igstAmount: p.igstAmount as number, cgstPct: p.cgstPct as number, sgstPct: p.sgstPct as number, igstPct: p.igstPct as number, isDeleted: p.isDeleted as boolean })),
    ];
