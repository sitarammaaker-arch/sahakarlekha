...
export interface SocietySettings {
  ...
  journalFirstWrites?: boolean;        // T-09 acceptance (journal-first-write): per-tenant WRITE cut. true = a voucher's save is a durable, VERIFIED journal append (persistEventAuthoritative); the vouchers table is then a best-effort projection (a table-write failure is recoverable — the journal is the truth), so the RULE-1 optimistic-rollback on the voucher path retires. Undefined/false = today's table-first save with rollback (default, byte-identical). Flipped per tenant ONLY after the T-09 read soak is clean (R3).
  fyCloseAuthorityRequired?: boolean;  // T-23 (UCAS CM-2 / CL-7): per-tenant. true = locking the FY is a FINALIZATION that needs a recorded board resolution (validated by authorizeFinalization: correct kind, reference, date, authorizer, and SoD — the authorizer must not be the admin locking). Undefined/false = today's behaviour, the FY locks without a recorded authority (default). Flipped per tenant.
  fyCloseAuthority?: string;           // T-23: the governance act that authorized the FY close, stamped at lock time (e.g. "board_resolution BR/2025-26/12 (2026-04-15) — Board of Directors"). The audit link from the locked year to the resolution that closed it.
  statutoryAppropriation?: boolean;    // T-20 (UCAS CM-1): per-tenant statutory appropriation posting. true = the society may post its year-end net-surplus appropriation through the canonical engine (effective-dated UCAS rates, exact paise, one balanced voucher via the ledger). Undefined/false = the legacy ad-hoc ReserveFund/ProfitDistribution float path only (default). Flipped per tenant; the legacy path is retired after soak.
